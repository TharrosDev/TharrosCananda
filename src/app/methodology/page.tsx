    ...
    <>
      ...
      <section className="section methodology-grid">
        <div className="methodology-main">
          ...
          <article id="demonstration-data"><h2>Demonstration data</h2><p>The V1 Market Explorer uses synthetic values to demonstrate the output structure. Those values are clearly labelled and must not be used as Canadian market statistics. Replacing them with live adapters should not require redesigning the interface.</p></article>
        ...
        <aside className="source-register"><h2>Initial source register</h2>{publicSources.map((source) => <a key={source.publisher} href={source.url} target="_blank" rel="noreferrer"><strong>{source.publisher}</strong><span>{source.purpose}</span><small>Open official source ↗</small></a>)}<p>References identify public publishers. They do not imply endorsement, partnership or validation of Tharros Canada.</p></aside>
      </section>
