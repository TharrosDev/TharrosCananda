  ...
  return (
    <>
      ...
      <section className="section methodology-grid">
        ...
        <aside className="source-register"><h2>Initial source register</h2>{publicSources.map((source) => <a key={source.publisher} href={source.url} target="_blank" rel="noreferrer"><strong>{source.publisher}</strong><span>{source.purpose}</span><small>Open official source ↗<span className="sr-only"> (opens in a new tab)</span></small></a>)}<p>References identify public publishers. They do not imply endorsement, partnership or validation of Tharros Canada.</p></aside>
      </section>
