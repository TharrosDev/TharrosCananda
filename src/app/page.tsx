...
const clientQuestions = [
  { title: "Assess a market", copy: "Review market structure, trade, geography and routes to market.", href: "/research-services#market-scan" },
  { title: "Find buyers or partners", copy: "Identify relevant buyers, importers, distributors and partners.", href: "/research-services#buyer-distributor" },
  { title: "Understand competitors", copy: "Compare offers, positioning, geography and channels.", href: "/research-services#competitor-intelligence" },
  { title: "Research a specific question", copy: "Scope a question across trade, defence, industry, energy or technology.", href: "/research-services#commissioned-research" },
] as const;
  ...
  return (
    <>
      <section className="home-intro">
        ...
        <div className="home-intro-foot"><p>Canada ↔ Europe</p><Link href="#start">Explore research areas <span aria-hidden="true">↓</span></Link></div>
      </section>

      <section className="section home-decisions" id="start">
        <div className="home-section-lead"><h2>Start with a question.</h2><p>Research is scoped around the information you need.</p></div>
      ...
      <section className="expertise-spread">
        <div className="expertise-spread-inner">
          <div className="expertise-statement"><p>Research areas</p><h2>Four connected fields.</h2><p>Work may span more than one area.</p></div>
      ...
      <section className="section home-services">
        <div className="home-section-lead"><h2>Commissioned research.</h2><p>Choose a defined service or request a custom scope.</p><Link className="text-link" href="/research-services">Services and pricing <ArrowIcon /></Link></div>
      ...
      <section className="research-threshold">
        <div><p>Public research</p><h2>{researchToShow.length ? "Selected releases." : "No publications yet."}</h2></div>
        <div>
          {researchToShow.length ? researchToShow.map((publication) => <Link key={publication.slug} href={`/research/${publication.slug}`}><strong>{publication.title}</strong><span>{publication.type} · {new Date(publication.publishedAt).toLocaleDateString("en-CA", { year: "numeric", month: "short", timeZone: "UTC" })}</span></Link>) : <p>Publications will appear here as they are released.</p>}
          <div className="threshold-links"><Link className="text-link" href="/research">Research archive <ArrowIcon /></Link><Link className="text-link" href="/methodology">Methodology <ArrowIcon /></Link></div>
        </div>
      </section>

      <section className="home-finale"><h2>Have a research question?</h2><Link className="button-primary" href="/request-research">Commission research <ArrowIcon /></Link></section>
    </>
  );
}
