...
export function LiveMarketExplorer({initialData=null}:{initialData?:TradeExplorerResponse|null}){
  ...
  return <section className="official-explorer" aria-labelledby="official-explorer-title" aria-busy={status==="loading"}>
    ...
    <div className="official-explorer-controls">
      ...
      <label><span>Commodity group</span><select value={commodity} onChange={(event)=>changeCommodity(event.target.value)} disabled={!data}>{(data?.options.commodities??[]).map((option)=><option key={option.id} value={option.id}>{option.label}</option>)}</select><small>Groups follow Statistics Canada metadata.</small></label>
    </div>
    {status==="error"&&<div className="official-data-error" role="alert"><strong>Statistics Canada data could not be loaded.</strong><p>{message}</p></div>}
    ...
    {status==="ready"&&data&&latest&&<>
      ...
      <div className="official-dataset-register"><div><h3>Open Government dataset search</h3><p>Related federal catalogue records for the selected commodity.</p></div><div>
