...
export const objectives = ["Enter the Canadian market","Enter the European market","Find buyers","Find a distributor or partner","Understand competitors","Validate demand","Map a sector or supply chain","Understand a policy or industrial development"] as const;
export type Objective = (typeof objectives)[number];

export type ResearchRequestPayload = {
  companyName: string; country: string; website: string; email: string; product: string; industry: string;
  description: string; hsCode: string; objectives: Objective[]; researchNeed: ResearchNeed | ""; context: string; consent: boolean;
};
...
export function validateResearchRequest(payload: Partial<ResearchRequestPayload>): ResearchRequestErrors {
  ...
  if (value("website") && normalizeWebsite(value("website")) === null) errors.website = "Enter a website address such as example.com, or leave this empty.";
  ...
  for (const key of textFields) if (!errors[key] && value(key).length > maxLengths[key]) errors[key] = `Keep this under ${maxLengths[key]} characters.`;
  ...
  if (payload.researchNeed && !researchNeeds.includes(payload.researchNeed)) errors.researchNeed = "Choose from the listed research options or leave this blank.";
...
export function parseResearchRequest(input: unknown): ParseResult {
  ...
  if (Array.isArray(raw.objectives) && raw.objectives.every((item) => typeof item === "string")) value.objectives = [...new Set(raw.objectives as Objective[])];
...
export function requestAsEmailBody(values: ResearchRequestPayload) {
  return [
    `Company: ${values.companyName} (${values.country})`, values.website && `Website: ${values.website}`, `Email: ${values.email}`,
    `Subject: ${values.product}`, values.industry && `Industry: ${values.industry}`, values.hsCode && `HS code: ${values.hsCode}`,
    values.description && `Question: ${values.description}`, `Objectives: ${values.objectives.join(", ")}`,
