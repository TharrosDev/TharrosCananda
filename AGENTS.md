...
- **`tests/css-guard.test.ts`** checks the motion tokens (`--dur-1`, `--dur-2`), allows scroll-linked `animation-timeline` only behind `prefers-reduced-motion: no-preference` and only for transform, translate, scale or stroke-dashoffset keyframes (no reveals; see DESIGN.md, Motion) and sets an 11.5px minimum screen font size outside `report.css`. Update the guard if the design intentionally changes.
