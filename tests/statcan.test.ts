...
const dimensions=[
  ...
  {dimensionPositionId:2,dimensionNameEn:"Free trade agreement",member:[{memberId:1,memberNameEn:"Total"},{memberId:7,parentMemberId:1,memberNameEn:"Canada-European Union Comprehensive Economic and Trade Agreement (CETA)"}]},
...
describe("Statistics Canada adapter helpers",()=>{
  it("applies Statistics Canada scalar-factor codes",()=>{expect(scalarMultiplier(0)).toBe(1);expect(scalarMultiplier(3)).toBe(1000);expect(scalarMultiplier(6)).toBe(1_000_000);});
  it("builds a 10-position WDS coordinate and fills unselected dimensions only with positively identified defaults",()=>{
    const coordinate=buildCoordinate(dimensions,new Map([[1,dimensions[0].member[1]],[2,dimensions[1].member[1]]]));
    ...
    expect(buildCoordinate([{dimensionPositionId:1,dimensionNameEn:"Single",member:[{memberId:4,memberNameEn:"Only"}]}],new Map())).toBe("4.0.0.0.0.0.0.0.0.0");
  });
