...
describe.each(allPublications.map((p) => [p.slug, p] as const))(
  ...
  (_, p) => {
    ...
    it("carries the metadata search, JSON-LD and the PDF keywords read", () => {
      ...
      expect(
        p.sources.length,
        "record-level sources feed the JSON-LD citation list",
      ).toBeGreaterThan(0);
...
describe("publication records", () => {
  ...
  it("hashes content and CSS so either change marks the PDF stale", () => {
    ...
    expect(
      reportSourceHash({ ...researchSpecimenPublication, title: "Changed" }, [".x{}", "markup"]),
    ).not.toBe(a);
  });
