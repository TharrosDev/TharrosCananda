...
import { fixture, fixturePath, fixturePdf } from "./fixture";
...
test("viewer draws every PDF page as a sheet with selectable text", async ({ page }) => {
  ...
  await expect(viewer.locator('.report-sheet[data-page="1"] .textLayer')).toContainText("Overview");
});
...
test("cite popover copies an APA citation with the stable reference URL", async ({
  ...
  expect(copied).toContain(`(Report No. ${ref})`);
  expect(copied).toContain(`/research/id/${ref}`);
});
...
test("share falls back to copy, and to a visible URL when copying is denied", async ({ page }) => {
  ...
  await expect(page.getByRole("textbox", { name: "Report link" })).toHaveValue(
    new RegExp(`/research/id/${ref}$`),
  );
});
...
test.describe("without JavaScript", () => {
  ...
  test("header, abstract, download and contents remain", async ({ page }) => {
    ...
    await expect(
      page.getByRole("navigation", { name: "Contents" }).getByRole("link", { name: /Limitations/ }),
    ).toBeVisible();
...
test("stable ID redirects permanently, case-insensitively; unknown IDs 404", async ({
  request,
}) => {
  const ok = await request.get(`/research/id/${ref.toLowerCase()}`, { maxRedirects: 0 });
  expect(ok.status()).toBe(308);
  expect(ok.headers()["location"]).toMatch(new RegExp(`${fixturePath}$`));
  expect((await request.get("/research/id/NOPE", { maxRedirects: 0 })).status()).toBe(404);
});

test("rapid zoom and resize never break, blank or duplicate pages, and reuse one worker", async ({
  page,
}) => {
  await page.addInitScript(() => {
  ...
  expect((await layer.innerText()).match(/Overview/g)).toHaveLength(1);
  const blank = await viewer.locator("canvas").evaluateAll(
    (canvases) =>
      canvases.filter((c) => {
  ...
  expect(blank).toBe(0);
  expect(
    await page.evaluate(() => (window as unknown as { __workers: number }).__workers),
...
test("contents jump to a section and mark it as current", async ({ page, isMobile }) => {
  ...
  await expect(link).toHaveAttribute("href", `${fixturePdf}#page=2`);
  await link.click();
...
test("sources, limitations and citation are readable page text", async ({ page }) => {
  ...
  await expect(page.locator("#limitations li")).toHaveCount(fixture.limitations!.length);
  await expect(page.locator("#cite .citation-text")).toContainText(ref);
  await expect(
    page.getByRole("link", { name: new RegExp(`More in ${area.name}`) }),
  ).toHaveAttribute("href", `/research?area=${area.slug}`);
});
