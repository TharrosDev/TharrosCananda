...
createServer((req, res) => {
  ...
  req.on("end", () => {
    ...
    if (url.pathname === "/api/v2/doc/doc") {
      ...
      return res.end(JSON.stringify({ articles: samples[topic].map(([title, domain, sourcecountry, language, hoursAgo], index) => ({
        title, domain, sourcecountry, language, seendate: gdeltStamp(hoursAgo), url: "https://" + domain + "/story-" + topic + "-" + index
      })) }));
    }
